...
if(tree=="ntphi"){
	if (varExp=="Bpt"){
		...
		double lolimit2[13]={init_mean*0.999,0.01,0.005,0.005,0.01,0.01,0,0,0,0,0.001,0.001,0};
		double hilimit2[13]={init_mean*1.0001,0.11,0.06,0.025,0.1,0.1,20,50,500,500,.999,.999,2};
		for (int i=0;i<13;i++){
			init[i]=init2[i];
...
*/
	...
	dsMC->plotOn(frameMC,Name(Form("dsMC_cut%d",_count)),Binning(nbinsmasshisto,init_mean*0.99,init_mean*1.02),MarkerSize(1.55),MarkerStyle(20),LineColor(1),LineWidth(4));
	if(pdf!="1gauss"){
