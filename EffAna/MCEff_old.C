...
void  MCEff_old(int DoTnP, int Rescale, TString meson_n){
...
//	BDT8->SetBranchAddress("BDT_pt_New_1_2",BDT_pt_1_2);
  ...
  const int yonlyBin = yBinVec.size() - 1;
	...
	TH1D * Eff1DRECOHisTnPUpMult = new TH1D("Eff1DRECOHisTnPUpMult","",nmBins_both,nmbinsvec.data());
...
//		BDT8->GetEntry(i);
      ...
      if ((Bgen[j] == 23333) && preselection
        ...
        if (TMath::Abs(By[j])<2.4 && passTracking && Bpt[j]>ptlow && Bpt[j]<pthigh) {
...
//////////
    ...
    // Track selection variation
		...
		TFile * foutSyst;
		if (DoTnP==1){
		foutSyst = new TFile(Form("%s/NewEff2DMaps/%sSyst.root",meson_n.Data(),meson_n.Data()),"RECREATE");
		foutSyst->cd();
		...
		TFile * foutSyst2D;
		if (DoTnP==1){
		foutSyst2D = new TFile(Form("%s/NewEff2DMaps/%sSyst2D.root",meson_n.Data(),meson_n.Data()),"RECREATE");
